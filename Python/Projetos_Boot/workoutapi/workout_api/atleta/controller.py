...
from workout_api.configs.dependencies import DatabaseDependency
...
async def post(
    ...
    if not categoria:
        raise HTTPException(
            ...
            detail=f'Categoria não {categoria_name} encontrada'
        )
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f'Centro não {centro_treinamento_name} encontrado'
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail='Ocorreu um erro ao inserir os '
        )
